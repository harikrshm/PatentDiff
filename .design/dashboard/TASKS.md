...
## Foundation
- [ ] **T1 · Dashboard grid shell + rename + funnel removal** _(establishes the
...
## Core UI
- [ ] **T2 · Block 1 — Trace Properties (static run UI + metadata)**: Move the
...
- [ ] **T3 · Block 2 — Eval failure-mode summary** _(risk-first: new data
...
- [ ] **T4 · Block 3 — Dimension heatmap (selectable row/col)**: Two dimension
  ...
  always shown._
- [ ] **T5 · Block 4 — Eval score vs KPI target**: For each eval kind, a readout
...
- [ ] **T6 · Block 5 — Metric trajectory**: Baseline → current → expected progress
...
- [ ] **T7 · Block 6 — Set KPI target**: PM inputs — target PASS-rate, target date,
...
## Interactions & States
- [ ] **T8 · Cross-block wiring + states**: Trace-set select (Block 1) refreshes
...
## Responsive & Polish
- [ ] **T9 · Responsive ~1024 + accessibility pass**: 2×3 grid → single stacked
